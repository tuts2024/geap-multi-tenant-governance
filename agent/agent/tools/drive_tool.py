...
class V1AlphaGcpAuthProvider(GcpAuthProvider):
    ...
    async def get_auth_credential(self, auth_config: AuthConfig, context: any) -> AuthCredential:
        ...
        if "drive" in auth_scheme.name or "google" in auth_scheme.name:
            ...
            # Check if Domain-Wide Delegation is enabled (user_id must be a Workspace email address)
            dwd_sa_email = os.environ.get("WORKSPACE_DWD_SA_EMAIL")
            is_email = "@" in user_id
            
            if dwd_sa_email and is_email:
                print(f"[WORKSPACE_DWD] Bypassing key files. Generating Domain-Wide Delegation via IAM Credentials API for {user_id} using service account {dwd_sa_email}...", flush=True)
                try:
                    # 1. Fetch credentials for our current running environment (GCP context / gcloud user credentials)
                    ...
                    import traceback
                    print(f"[WORKSPACE_DWD] [ERROR] Dynamic signJwt DwD failed: {e}. Falling back to impersonation.", flush=True)
                    traceback.print_exc()
            
            # MODEL 1: Tenant-Specific Service Account Impersonation (Default Demo Mode)
