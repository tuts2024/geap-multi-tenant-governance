...
async def get_connector_credential(tool_context, auth_scheme) -> AuthCredential | None:
    ...
    if "drive" in auth_scheme.name or "google" in auth_scheme.name:
        ...
        try:
            ds_client = datastore.Client(project=config.PROJECT_ID)
            key = ds_client.key("UserConfiguration", user_id)
            user_config = ds_client.get(key)
            if user_config:
                ...
                print(f"[AUTH_UTILS] Found user config for {user_id}: Strategy={auth_strategy}", flush=True)
        except Exception as e:
            print(f"[AUTH_UTILS] Error reading user config from Datastore: {e}", flush=True)
